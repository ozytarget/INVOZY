...
import { createEstimate } from '@/lib/estimates-service';
...
export default function CreateEstimatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.client_name || !formData.project_title || !formData.amount) {
      ...
      await createEstimate(user.id, estimateData);
      router.push('/dashboard/estimates');
    } catch (err: any) {
      console.error('Error creating estimate:', err);
      const errorMessage = err?.message || err?.toString() || 'Error desconocido';
      alert(`Error al crear estimado: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-8">Nuevo Estimado</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-8 max-w-2xl">
        ...
        <div className="mt-8 flex space-x-4">
          ...
          >
            {loading ? 'Creando...' : 'Crear Estimado'}
          </button>
          <button
            type="button"
            onClick={() => router.back()}
